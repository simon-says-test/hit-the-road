...
- Going off the paved road (the dirt shoulder between the paved edge and the canyon wall) costs speed only — extra drag, no health cost — rather than simply blocking movement. Driving over a rough-terrain or oil-slick patch costs neither. The canyon wall *beyond* that shoulder is the one thing in the environment that costs health on contact (see the next bullet and Road & hazards) — it's also a hard boundary the off-road shoulder isn't, so a car can't actually drive through/past it.
- Colliding with an enemy vehicle never costs the player health directly — it jolts the player's current speed, scaled by how fast the two were closing on each other at the moment of impact (a fast catch-up from a chaser jolts you harder than barely clipping a slow one). Being rammed from behind lurches you forward; being clipped from the side knocks you back — either way it registers even from a standstill, not just as a dent in existing speed. That same impact also damages the *enemy*, scaled the same way but biased toward the speed jolt over raw damage — a glancing low-speed bump barely scratches it, a hard fast ram can still one-shot a weak car, and a brief per-enemy cooldown after each ram stops one continuous overlap from re-damaging it every frame before physics can separate the two cars. Contact is physical, not just a number changing in the background — a car that survives a ram visibly bounces off rather than gliding through the player's position. See Game over & restart for when a run actually ends — only hazards, enemy gunfire, and canyon-wall impacts cost the player health (off-road driving itself doesn't — see the bullet above), not ramming itself.
- Hitting a canyon wall costs a one-time speed cut the instant contact begins, on top of the continuous scraping drag while still in contact — trading some of the old flat impact damage for a slowdown that actually reads as hitting something solid, not just a number ticking down. (It also costs health — see the off-road bullet above and Road & hazards.)
- The following camera is zoomed out a little (further still on a touch device) and biases its view toward wherever the car is currently heading, scaled by speed — more of the upcoming road is visible at speed/in a turn than a plain centered follow would show, giving more time to react.
...
- **Turret machine gun** — aimed directly with the mouse/pointer (or, on a touch device, a tap/hold anywhere on the field, even while the other thumb is still on the virtual joystick), very high ammo pool, low damage per hit, short cooldown; fired by clicking (not Space, since Space is the other two weapons' fire key) or by that same tap/hold on touch. A crosshair follows the pointer/finger so it's clear exactly where a shot will land, separate from the spread-cone meter ahead of the car. Accuracy spread widens with steering, while off-road, and sharply while drifting — it rewards driving smoothly rather than aggressively while it's equipped. Small, fast, cyan tracer.
...
- Enemies have a health value; in the default **Arcade** mode they're destroyed at zero, whether by weapon fire or by ramming them, and destroying one has a chance to drop a pickup (health, ammo, or a boost — see Pickups) and adds to the placeholder distance/score number (see Scoring & progression). In **Repair** mode they instead stop and repair at zero health — see Game over & restart for both modes.
- The player's own vehicle has health/armor; enemy weapon hits, canyon-wall impacts, and hazards reduce it. Off-road driving (the shoulder short of the wall) and ramming an enemy vehicle do not — see Vehicle handling above.
...
- Completing a lap restores a small flat amount of health (`LAP_HEALTH_BONUS`) — for the player and every rival alike, clamped to each car's own max, so it's a reward for surviving a lap rather than a way to stack health above full.
...
- The paved road is a constant width along the whole loop; off-road driving (the dirt shoulder beyond the paved edge, short of the canyon wall) is penalized by drag (slowed) only — no health cost — rather than simply blocked.
- **Rocky canyon walls** flank the road beyond that dirt shoulder, at a distance that meanders along the length of the loop rather than staying fixed — sometimes a wide shoulder, sometimes a tight squeeze. The walls are a hard boundary: a car (player or rival) pushed into one can't drive through it, unlike the dirt shoulder in front of them. Drag is continuous for as long as contact lasts (steeper than plain off-road), and health damage is a one-time hit on the frame contact begins, scaled by how fast the car was going at that instant — scraping along a wall while steering back onto the road doesn't keep costing health, only the initial impact does. The wall is the *only* terrain that costs health; the dirt shoulder in front of it never does. No enemy archetype approaches from the side any more (see Enemies) — everything closes or falls back along the same ahead/behind axis as the player, now measured along the loop rather than straight up the screen.
- A small discrete rock/debris obstacle (distinct from the continuous rough/oil patches below) costs a one-time bump on contact — speed loss weighted over raw damage, the same trade as the wall/ram impacts above — then despawns. Sized to read clearly as a real obstacle rather than a barely-visible pebble.
...
Two modes, chosen once on the intro screen (defaults to Arcade on every fresh page load) and carried through every restart for the rest of that session:

- **Arcade** (default) — a car (player or rival) that reaches zero health, from a canyon wall, ramming, or enemy fire, is destroyed.
- **Repair** — a car that reaches zero health instead stops dead (no driving/AI movement, no firing) for a few seconds, with an on-screen "Repairing…" countdown for the player and a dulled tint for any car mid-repair, then resumes at half its max health rather than being destroyed. It can still be hit (and re-trigger another repair later) while otherwise immobile.

A run ends one of two ways:

- The player's own vehicle is destroyed — Arcade mode only; in Repair mode the player's car repairs instead and the run continues — → **game over** screen.
...
Either way, the screen shows the final distance → restart (Space or tap), generating a fresh track in the same mode. It also shows the best distance reached across all previous runs of either outcome (persisted locally in the browser, not server-side) and flags it explicitly when the just-finished run set a new one.

A **mid-race restart** is also available without waiting for either ending: once the player's car has been stopped (or nearly so) for a few continuous seconds, a RESTART button appears in the HUD (also bound to the R key) — tap/click or press R to immediately restart in the same mode. It only appears once actually stationary, specifically so it can't be hit by accident mid-drive.
